...
Unpaid `attempting` sessions can also be closed. The local close action no longer waits for provider status or a polling lease; it atomically checks the latest local state and makes reconciliation due. An age-only review flag remains recorded but does not prevent dismissal. Confirmed payments, active processing, unknown creation and financial mismatches remain protected. The reported Bank Transfer attempt was locally closed on request, with a private backup, unchanged balance and reconciliation retained.
