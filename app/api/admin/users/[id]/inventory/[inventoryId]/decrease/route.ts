...
export async function POST(
  ...
  try {
    ...
    const meta = await getRequestMeta(`/api/admin/users/${id}/inventory/${inventoryId}/decrease`);
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Unable to decrease quantity." }, { status: 400 });
  }
}
