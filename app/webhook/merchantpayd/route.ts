...
export async function POST(request:Request) {
  ...
  if (!config.webhookSecret) return NextResponse.json({error:"Webhook not configured."},{status:503});
  const raw=new Uint8Array(await request.arrayBuffer());
  if (raw.byteLength>262144) return NextResponse.json({error:"Payload too large."},{status:413});
  if (!verifyMerchantSignature(raw,request.headers.get('x-webhook-signature'),config.webhookSecret)) return NextResponse.json({error:"Invalid signature."},{status:401});
  try {
