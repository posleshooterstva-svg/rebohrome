...
export async function createMerchantPayment(input:{userId:string;kind:"purchase"|"deposit";key:string;amount?:number;paymentMethod?:MerchantMethodCode;currency?:'USD'|'EUR';eurUsdRate?:number;items?:Array<{productId:string;quantity:number;deliveryType:DeliveryType}>}) {
  ...
  if(input.currency!==methodCurrency(paymentMethod))throw new Error(`${MERCHANTPAYD_METHODS[paymentMethod]} requires ${methodCurrency(paymentMethod)}.`);
  const rate=paymentMethod==='banking'?await getExchangeRate('EUR','USD'):1;
  if(paymentMethod==='banking'&&input.eurUsdRate!==rate)throw new Error('The exchange rate changed. Refresh the page and review the amount before paying.');
  const terms=paymentTerms(paymentMethod,input.kind,order?.amountMinor ?? minorUnits(input.amount),rate);
  const amountMinor=terms.usdAmountMinor;
  ...
  const snapshot={user:{name:user.name||user.username,email:user.email},order,paymentTerms:terms};
  const fingerprint=createHash('sha256').update(JSON.stringify({kind:input.kind,amountMinor,items:input.items||null,paymentMethod,paymentTerms:terms})).digest('hex');
