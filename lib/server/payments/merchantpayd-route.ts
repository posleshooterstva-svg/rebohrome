...
export async function merchantCreateRoute(request:Request,kind:"deposit"|"purchase") {
  ...
  try {
    ...
    const result=await createMerchantPayment({userId:session.userId,kind,key:request.headers.get('idempotency-key')||'',amount:payload.amount,items:payload.items,paymentMethod:payload.paymentMethod});
